        ...
        <section className="py-20 bg-slate-50">
          <div className="container mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {streams.map((stream, index) => <Card key={index} className="hover:shadow-lg transition-shadow">
                  <CardHeader>
                    <div className="flex items-center justify-between mb-2">
                      <span className={`px-2 py-1 text-xs rounded-full ${stream.status === 'live' ? 'bg-red-100 text-red-600' : stream.status === 'upcoming' ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-600'}`}>
                  ...
                  <CardContent>
                    ...
                    <Button className="w-full" variant={stream.status === 'live' ? 'default' : 'outline'}>
                      {stream.status === 'live' ? 'Join Live' : stream.status === 'upcoming' ? 'Set Reminder' : 'Watch Recording'}
                    </Button>
                  </CardContent>
                </Card>)}
            </div>
